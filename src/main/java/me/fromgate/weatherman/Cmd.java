...
public class Cmd implements CommandExecutor{
	...
	private boolean executeCheckCmd (CommandSender sender, String [] args){
		...
		else u.printMSG(sender, "msg_biomeloc2", BiomeTools.colorBiomeName(BiomeTools.biome2Str(b1)),BiomeTools.colorBiomeName(BiomeTools.biome2Str(b2)));
		return true;
	}
	...
	private boolean executeListCmd(CommandSender sender, String[] args) {
		...
		if (arg.equalsIgnoreCase("tree")||arg.equalsIgnoreCase("tree")) return u.returnMSG(true,sender, "msg_treelist",Forester.getTreeStr(sender));
	...
	private boolean executeGiveCmd(CommandSender sender, String[] args) {
		...
		String arg = args.length>1 ? args [1] : "";
		Player player = (Player) sender;
		if (arg.equalsIgnoreCase(Brush.BIOME.name())) ItemUtil.giveItemOrDrop(player, ItemUtil.parseItemStack(Brush.BIOME.getItemStr()));
		else if (arg.equalsIgnoreCase(Brush.WOODCUTTER.name())) ItemUtil.giveItemOrDrop(player, ItemUtil.parseItemStack(Brush.WOODCUTTER.getItemStr()));
		else if (arg.equalsIgnoreCase(Brush.DEPOPULATOR.name())) ItemUtil.giveItemOrDrop(player, ItemUtil.parseItemStack(Brush.DEPOPULATOR.getItemStr()));
		else if (arg.equalsIgnoreCase(Brush.FORESTER.name())) ItemUtil.giveItemOrDrop(player, ItemUtil.parseItemStack(Brush.FORESTER.getItemStr()));
		else arg = "";
	...
	public boolean ExecuteCmd(Player p, String cmd, String arg1, String arg2){
		if (cmd.equals("weather_player")){
			...
			if ((player==null)||(!player.isOnline())) return u.returnMSG(true, p, "wth_unknownplayer",'c','4',arg1);

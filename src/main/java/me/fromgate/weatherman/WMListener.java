	...
	public void onSignChange (SignChangeEvent event) {
		...
		if (ChatColor.stripColor(event.getLine(1)).equalsIgnoreCase("[biome]"))
			if (!p.hasPermission("weatherman.sign")) event.setLine(1, "{biome}");
				...
				if (!BiomeTools.isBiomeExists(event.getLine(0))) event.setLine(0, BiomeTools.biome2Str(event.getBlock().getBiome()));
				...
				else if (l2.isEmpty()) event.setLine(2, ChatColor.BLUE+"radius="+Integer.toString(plg.defaultRadius));
				else {
					event.setLine(2, ChatColor.BLUE+l2);
					if (WMWorldEdit.isWG()){
						if (!WMWorldEdit.isRegionExists(event.getBlock().getWorld(), l2)) u.printMSG(p, "wg_unknownregion",'c','4',l2);
					} else {
				...
				if(event.getLine(3).isEmpty()||
						((!event.getLine(3).isEmpty())&&(!BiomeTools.isBiomeExists(event.getLine(3))))) event.setLine(3, BiomeTools.biome2Str(plg.defaultBiome));
				...
				if (!ChatColor.stripColor(sign.getLine(1)).equalsIgnoreCase("[biome]")) return; 
					...
					if (BiomeTools.isBiomeExists(b1)&&BiomeTools.isBiomeExists(b2)){
						...
						if (mode>=0){
							...
							switch (mode){
							case 0: BiomeTools.setBiomeRadius(null,sign.getLocation(), biome, radius,null); break;
							case 1: BiomeTools.floodFill(null, sign.getLocation(), biome); break;
							case 2: BiomeTools.setBiomeArea(null, loc1, loc2, biome,null);break;
							}
							sign.update(true);
						}
					}
				} else u.log("Something wrong with WeatherMan-sign: ["+ChatColor.stripColor(sign.getLine(0))+"|"+ChatColor.stripColor(sign.getLine(1))+
